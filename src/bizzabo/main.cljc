(ns bizzabo.main
  "Kotodama WASM entrypoint for the Bizzabo clean-room actor (L4) — Clojure port.
  ...
  facts (`bizzabo.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "bizzabo")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"bizzabo.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "bizzabo-compat" :tier tier :entities entities} 200])
...
(defn start! [] :bizzabo-compat/ready)
